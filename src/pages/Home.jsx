...
const [loading,setLoading]= useState (true);
  ...
  useEffect(() => {
    fetchCategories().then((res)=> setGategories(res.data.categories)).catch((err)=> console.log(err)).finally(()=> setLoading(false));
  },[]);
  ...
  return (
    ...
    {categories.map((cat)=>(
      <Link to={`/categoryy/${cat.strCategory}`} key={cat.idCategory} className='group cursor-pointer'>
        <img src={cat.strCategoryThumb} alt={cat.strCategory} className='w-full rounded-lg shadow 
